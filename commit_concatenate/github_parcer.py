...
def parse_commit_date(URL: str,
        ...
        json = parse_page_response(URL=URL, user=None, page=None,
...
def git_to_datetime(git_date: str) -> datetime.date | None:
    ...
    try:
        unix_date = datetime.datetime.strptime(git_date_fixed,
                                               '%Y-%m-%d').date()
    except ValueError:
        return None
    return unix_date
...
def get_data(json: list[dict],
             requests_session: requests.Session | None = None) -> dict:
    dates_list = []
    executor = concurrent.futures.ProcessPoolExecutor(10)
    tasks = parse_links_from_json(json)
    dates_list = [executor.submit(parse_commit_date, commit,
                                  requests_session) for commit in tasks]
...
def try_request(user: str | None, page: int | None,
                request_session: requests.Session | None,
                URL: str = BASE_URL) -> requests.Response | None:
    count_tries = 0
    response = None
    error = False
    while True:
...
def parse_page_response(user: str | None,
                       page: int | None,
                        request_session: requests.Session | None,
                       URL: str = BASE_URL) -> list | dict:
    response = try_request(user=user, page=page,
...
def parse(user: str = DEFAULT_USER, URL: str = BASE_URL)-> dict:
    ...
    while len(json) > 0 and page < 2:
        ...
        page += 1
        json = parse_page_response(user=user, page=page,
                                   URL=URL, request_session=None)
