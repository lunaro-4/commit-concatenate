...
from commit_concatenate.github_parcer import *
...
def test_parse_page(run_around_tests):
    response = parse_page_response(URL= 'file://' + TEST_PAGE_URL,
...
def test_parse_commit_date(run_around_tests):
    date_1 = parse_commit_date(URL= 'file://' + TEST_COMMIT_1_URL, requests_session=run_around_tests)
    date_2 = parse_commit_date(URL= 'file://' + TEST_COMMIT_2_URL, requests_session=run_around_tests)
...
def test_git_to_datetime():
    ...
    expect_date_1 = datetime.datetime\
            .strptime('2024-05-26', '%Y-%m-%d').date()
    expect_date_2 = datetime.datetime\
            .strptime('2021-02-23', '%Y-%m-%d').date()
...
def test_get_data(run_around_tests):
    links_to_test_commits = ['file://' +TEST_COMMIT_1_URL,
                             'file://' +TEST_COMMIT_2_URL]
    github_parcer.parse_links_from_json = MagicMock(return_value=links_to_test_commits)
